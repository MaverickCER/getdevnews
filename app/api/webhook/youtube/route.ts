 ...
 * Example: http://localhsot:3000/api/webhook/youtube?hub.challenge=challenge GET
 * 
 * GET Endpoint for validating youtube webhook subscriptions.
 * POST Endpoint for youtube to add videos to the home page via pubsubhubbub.
 * This POST endpoint is triggered when a YouTube channel publishes a new video.
...
export async function handler(request: NextApiRequest, response: NextApiResponse) {
  ...
  try {
    if (request.method === 'GET') {
      ...
      response.status(204).send(hub_challenge);
